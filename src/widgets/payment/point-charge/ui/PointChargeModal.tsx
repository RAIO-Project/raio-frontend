    ...
    >
      <div className="w-full max-w-md rounded-[2rem] border border-border bg-bg-2 p-6 shadow-2xl animate-slide-up">
        <div className="mb-5 flex items-start justify-between">
          <div>
            <h2 className="text-lg font-black">포인트 충전</h2>
            <p className="mt-1 text-xs text-white/40">현재 보유 {formatPoint(balance)}</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {OPTIONS.map((point) => (
            <button
              ...
              className={`rounded-2xl border p-3 text-sm font-black ${selected === point ? 'border-accent bg-accent/10 text-accent' : 'border-border bg-bg-3 text-white/55'}`}
            >
              {formatPoint(point)}
            </button>
          ))}
        </div>
        <button onClick={charge} className="mt-5 w-full rounded-2xl bg-accent py-3.5 text-sm font-black text-black">
          {formatPoint(selected)} 충전하기
        </button>
