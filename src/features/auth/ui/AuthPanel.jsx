...
const initialForm = { email: '', password: '', passwordConfirm: '', nickname: '', phoneNumber: '' }
...
export function AuthPanel({ mode }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setError(isRegister ? '회원가입에 실패했습니다.' : '이메일 또는 비밀번호가 올바르지 않습니다.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="w-full max-w-md rounded-[2rem] border border-border bg-bg-2/90 p-7 shadow-2xl backdrop-blur">
      ...
      <form onSubmit={submit} className="space-y-4">
        <Field label="이메일" value={form.email} onChange={update('email')} type="email" placeholder="example@raio.tv" />
        {isRegister && <Field label="닉네임" value={form.nickname} onChange={update('nickname')} placeholder="방송/채팅에서 보일 이름" />}
        {isRegister && <Field label="휴대폰 번호" value={form.phoneNumber} onChange={update('phoneNumber')} placeholder="010-0000-0000" />}
        <Field label="비밀번호" value={form.password} onChange={update('password')} type="password" placeholder="8자 이상" />
        {isRegister && <Field label="비밀번호 확인" value={form.passwordConfirm} onChange={update('passwordConfirm')} type="password" placeholder="비밀번호 재입력" />}
