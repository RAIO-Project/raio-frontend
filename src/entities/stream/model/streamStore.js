...
export const CATEGORIES = ['전체', '게임', '먹방', '소통', '음악', '공부', '스포츠', '창작']
...
export const useStreamStore = create((set, get) => ({
  ...
  getStreams: () => {
    const { streams, category, query } = get()
    return streams.filter((stream) => {
      const matchCategory = category === '전체' || stream.category === category
  ...
  getStreamById: (id) => get().streams.find((stream) => String(stream.id) === String(id)) ?? get().streams[0],
}))
