  ...
  return (
    ...
    >
      <div className="relative aspect-video overflow-hidden bg-gradient-to-br from-bg-4 to-bg-5">
        <div className="absolute inset-0 flex items-center justify-center text-6xl opacity-40 transition group-hover:scale-110 group-hover:opacity-60">
          {stream.emoji}
        </div>
      ...
      <div className="space-y-2 p-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {stream.tags.map((tag) => (
            <span key={tag} className="rounded-full bg-bg-4 px-2 py-0.5 text-[10px] font-bold text-white/35">
              #{tag}
            </span>
          ))}
        </div>
